...
class Organize_model extends CI_Model
{
    ...
    public function addEmitente($nome, $cnpj, $ie, $cep, $logradouro, $numero, $bairro, $cidade, $uf, $telefone, $email, $logo, $regimeTributario, $ibge)
    {
        ...
        $this->db->set('regime_tributario', $regimeTributario);
        $this->db->set('ibge', $ibge);
        return $this->db->insert('emitente');
    }

    public function editEmitente($id, $nome, $cnpj, $ie, $cep, $logradouro, $numero, $bairro, $cidade, $uf, $telefone, $email, $regimeTributario, $ibge)
    {
        $this->db->set('nome', $nome);
        $this->db->set('cnpj', $cnpj);
        $this->db->set('ie', $ie);
    ...
    public function editCertificado($id, $certificado, $senha)
    {
        $this->db->set('path_certificado', $certificado);
        $this->db->set('senha_certificado', $senha);
        $this->db->where('id', $id);
        return $this->db->update('emitente');
    }
    ...
    {
        $sefaz = $this->db->get('sefaz')->row();
        $sefaz->data_alteracao = date_create_from_format('Y-m-d H:i:s', $sefaz->data_alteracao);
        return $sefaz;
    }

    public function saveSefaz($dados)
    {
        $this->db->set('ambiente', $dados['sefaz_ambiente']);
        $this->db->set('uf', $dados['sefaz_uf']);
        $this->db->set('data_alteracao', date_format(new DateTime(), 'Y-m-d H:i:s'));
        $this->db->where('id', 1);
        $this->db->update('sefaz');
    }
}
