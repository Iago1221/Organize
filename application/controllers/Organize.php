...
class Organize extends MY_Controller
{
    ...
    {
        $this->data['ordens'] = $this->Organize_model->getOsAbertas();
        $this->data['ordens1'] = $this->Organize_model->getOsAguardandoPecas();
        $this->data['ordens_andamento'] = $this->Organize_model->getOsAndamento();
        $this->data['produtos'] = $this->Organize_model->getProdutosMinimo();
        $this->data['os'] = $this->Organize_model->getOsEstatisticas();
        $this->data['estatisticas_financeiro'] = $this->Organize_model->getEstatisticasFinanceiro();
        $this->data['financeiro_mes_dia'] = $this->Organize_model->getEstatisticasFinanceiroDia($this->input->get('year'));
        $this->data['financeiro_mes'] = $this->Organize_model->getEstatisticasFinanceiroMes($this->input->get('year'));
        $this->data['financeiro_mesinadipl'] = $this->Organize_model->getEstatisticasFinanceiroMesInadimplencia($this->input->get('year'));
        $this->data['menuPainel'] = 'Painel';
        $this->data['view'] = 'Organize/painel';
        return $this->layout();
    }

    public function minhaConta()
    {
        $this->data['usuario'] = $this->Organize_model->getById($this->session->userdata('id_admin'));
        $this->data['view'] = 'Organize/minhaConta';
        return $this->layout();
    }

    public function alterarSenha()
    {
        $current_user = $this->Organize_model->getById($this->session->userdata('id_admin'));

        if (!$current_user) {
            $this->session->set_flashdata('error', 'Ocorreu um erro ao pesquisar usuário!');
            redirect(site_url('Organize/minhaConta'));
        }
        ...
        $result = $this->Organize_model->alterarSenha($senha);
    ...
    public function pesquisar()
    {
        ...
        $data['results'] = $this->Organize_model->pesquisar($termo);
        $this->data['produtos'] = $data['results']['produtos'];
        $this->data['servicos'] = $data['results']['servicos'];
        $this->data['os'] = $data['results']['os'];
        $this->data['clientes'] = $data['results']['clientes'];
        $this->data['view'] = 'Organize/pesquisa';
        return $this->layout();
    }
    ...
    public function emitente()
    {
        ...
        $this->data['dados'] = $this->Organize_model->getEmitente();
        $this->data['view'] = 'Organize/emitente';
        return $this->layout();
    }

    public function do_upload_certificado()
    {
        $path = FCPATH . 'application\files\certificados\\';
        $uploadfile = $path . basename($_FILES['certfile']['name']);
        if (move_uploaded_file($_FILES['certfile']['tmp_name'], $uploadfile)) {
            return $uploadfile;
        }
    }
    ...
    public function do_upload($path = null)
    {
        ...
        $image_upload_folder = $path ?: FCPATH . 'assets/uploads';
        ...
        $this->upload_config = [
            ...
            'allowed_types' => 'png|jpg|jpeg|bmp|svg|pfx',
            'max_size' => 2048,
            'remove_space' => true,
            'encrypt_name' => true,
        ];
    ...
    public function cadastrarEmitente()
    {
        ...
        if ($this->form_validation->run() == false) {
            ...
            redirect(site_url('Organize/emitente'));
        } else {
            ...
            $retorno = $this->Organize_model->addEmitente($nome, $cnpj, $ie, $cep, $logradouro, $numero, $bairro, $cidade, $uf, $telefone, $email, $logo, $regimeTributario, $ibge);
            if ($retorno) {
    ...
    public function editarEmitente()
    {
        ...
        if ($this->form_validation->run() == false) {
            ...
            $retorno = $this->Organize_model->editEmitente($id, $nome, $cnpj, $ie, $cep, $logradouro, $numero, $bairro, $cidade, $uf, $telefone, $email, $regimeTributario, $ibge);
            if ($retorno) {
    ...
    public function editarLogo()
    {
        ...
        $retorno = $this->Organize_model->editLogo($id, $logo);
        if ($retorno) {
            $this->session->set_flashdata('success', 'As informações foram alteradas com sucesso.');
    ...
    public function editarCertificado()
    {
        ...
        $id = $this->input->post('id');
        if ($id == null || !is_numeric($id)) {
            $this->session->set_flashdata('error', 'Ocorreu um erro ao tentar alterar o certificado.');
            redirect(site_url('Organize/emitente'));
        }
        $this->load->helper('file');
        delete_files(FCPATH . 'application\files\certificados\\');
        $certificado = $this->do_upload_certificado();
        
        $retorno = $this->Organize_model->editCertificado($id, $certificado, $_POST['senhaCertificado']);
        if ($retorno) {
            $this->session->set_flashdata('success', 'As informações foram alteradas com sucesso.');
            log_info('Alterou a logomarca do emitente.');
        } else {
    ...
    public function uploadUserImage()
    {
        ...
        $usuario = $this->Organize_model->getById($id);
        ...
        $imageUserPath = $image;
        $retorno = $this->Organize_model->editImageUser($id, $imageUserPath);
    ...
    public function emails()
    {
        ...
        $this->data['configuration']['base_url'] = site_url('Organize/emails/');
    ...
    public function excluirEmail()
    {
        ...
        if ($id == null) {
            ...
            redirect(site_url('Organize/emails/'));
        }
    ...
    public function configurar()
    {
        ...
        $sefaz = $this->Organize_model->getSefaz();
        $this->data['configuration']['sefaz_ambiente'] = $sefaz->ambiente;
        $this->data['configuration']['sefaz_uf'] = $sefaz->uf;
        ...
        if ($this->form_validation->run() == false) {
            ...
            $dataSefaz = [
                'sefaz_ambiente' => $this->input->post('sefaz_ambiente'),
                'sefaz_uf' => $this->input->post('sefaz_uf')
            ];

            $this->Organize_model->saveSefaz($dataSefaz);

            if ($this->Organize_model->saveConfiguracao($data) == true) {
                $this->session->set_flashdata('success', 'Configurações do sistema atualizadas com sucesso!');
                redirect(site_url('Organize/configurar'));
            } else {
                $this->data['custom_error'] = '<div class="alert">Ocorreu um errro.</div>';
            }
        }

        $this->data['view'] = 'Organize/configurar';
    ...
    public function atualizarBanco()
    {
        ...
        return redirect(site_url('Organize/configurar'));
    }

    public function atualizarOrganize()
    {
        if (!$this->permission->checkPermission($this->session->userdata('permissao'), 'cSistema')) {
            $this->session->set_flashdata('error', 'Você não tem permissão para configurar o sistema');
            redirect(base_url());
        ...
        if (!$this->github_updater->has_update()) {
            $this->session->set_flashdata('success', 'Seu Organize já está atualizado!');
        ...
        if ($success) {
            $this->session->set_flashdata('success', 'Organize atualizado com sucesso!');
        } else {
            $this->session->set_flashdata('error', 'Erro ao atualizar Organize!');
        }
    ...
    public function calendario()
    {
        ...
        $allOs = $this->Organize_model->calendario(
            $start,
            $end,
            $status
        );
