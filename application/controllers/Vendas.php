...
class Vendas extends MY_Controller
{
    ...
    public function adicionar()
    {
        ...
        if ($this->form_validation->run('vendas') == false) {
            ...
            if (is_numeric($id = $this->vendas_model->add('vendas', $data, true))) {
                ...
                redirect(base_url() . 'vendas/editar/' . $id);
            } else {
    ...
    public function visualizar()
    {
        ...
        $this->data['result'] = $this->vendas_model->getById($this->uri->segment(3));
        $this->data['produtos'] = $this->vendas_model->getProdutos($this->uri->segment(3));
        $this->data['emitente'] = $this->Organize_model->getEmitente();
    ...
    public function gerarNotaFiscal() {
        ...
        $vendaId = $this->uri->segment(3);
        $this->load->model('vendas_model');
        $this->load->model('NotaFiscal_model');

        $editavel = $this->vendas_model->isEditable($vendaId);
        if ($editavel) {
            //$this->session->set_flashdata('error', 'Erro ao tentar gerar a nota fiscal. A venda não foi faturada.');
            //redirect(('vendas/gerenciar/'));
        }

        if ($this->NotaFiscal_model->getByVenda($vendaId)) {
            $this->session->set_flashdata('error', 'Já existe uma nota fiscal para esta venda.');
            redirect(('vendas/gerenciar/'));
        }

        $data = new stdClass();
        $data->venda = $vendaId;
        $data->serie = $this->NotaFiscal_model->getSerie();
        $data->numero = $this->NotaFiscal_model->getNumero($data->serie);
